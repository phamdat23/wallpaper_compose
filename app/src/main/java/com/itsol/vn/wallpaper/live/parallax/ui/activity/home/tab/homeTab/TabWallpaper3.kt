...
@Composable
fun TabWallpaper3(modifier: Modifier = Modifier, titleAttr: String) {
    val navController = LocalNavController.current
    val viewModel = hiltViewModel<MainViewModel>()
    val context = LocalContext.current
    var arrt by rememberSaveable {
        mutableStateOf(Constants.IMAGE4K)
    }
    val isLoading by viewModel.loading.collectAsState()
    val listWallpaper by viewModel.wallpaperByAttr.collectAsState()
    LaunchedEffect(true) {
        AdsManager.logEvent(context, "Home_tab_${Constants.IMAGE4K}_screen")
        Common.isTypeSetWallpaper= Constants.IMAGE4K
        viewModel.getWallpaperByAttr(Constants.IMAGE4K)
    }
    AdsManager.ScreenNameLogEffect("Home_tab_${Constants.IMAGE4K}_screen")
